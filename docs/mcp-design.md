...
1. ✅ **Resolved.** **Hermes Agent `mcpServers` config format.** Confirmed via real Hermes smoke (see [`integration-roadmap.md`](./integration-roadmap.md) Phase 3 verification record). Hermes uses `~/.hermes/config.yaml` with a top-level `mcp_servers:` key, each server taking `command`, `args`, and an `env:` block (Hermes only forwards an allowlist of parent-shell vars, so `IFLOW_API_KEY` must live in the per-server `env:` block). The working YAML recipe is documented in [`packages/search-mcp/README.md`](../packages/search-mcp/README.md) → "Hermes Agent".
