...
| **Coze** | `@iflow-ai/search-openapi`'s `/openapi.coze.json` (OpenAPI 3.0.3, no security advertised, concrete inline `data` schemas, `servers[0].url` from `IFLOW_OPENAPI_PUBLIC_URL`) | No new package — same OpenAPI package | Coze-flavored route landed in unreleased pre.2 source; real-platform re-smoke pending operator approval | Coze cannot parse OpenAPI 3.1 and rejects spec-declared `BearerAuth` at execute time. Auth lives at the tunnel / reverse-proxy layer, not in `IFLOW_OPENAPI_AUTH_TOKEN`. See [`platform-smokes.md`](./platform-smokes.md). | <https://www.coze.com/docs/> · <https://spec.openapis.org/oas/latest.html> |
